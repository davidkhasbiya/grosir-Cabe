...
export default function PantauStokPage() {
  ...
  return (
    <div className="space-y-10">
      {/* Header - Gunakan text-foreground agar berubah warna */}
      <h1 className="text-4xl font-serif font-bold text-foreground">
        {query ? (
          <>Hasil untuk <span className="text-red-600 dark:text-red-400">"{query}"</span></>
        ) : (
          <>Pantau <span className="text-red-600 dark:text-red-400">Stok Nasional</span></>
        )}
      </h1>
        ...
        {filteredStok.length > 0 ? (
          filteredStok.map((item) => (
            <Link key={item.id} href={`/dashboard/riset/${item.id}`} className="group block">
              {/* Card - Gunakan bg-card dan border-border */}
              <Card className="p-6 rounded-[2.5rem] bg-card border-border shadow-sm hover:shadow-xl transition-all cursor-pointer overflow-hidden">
                ...
                {/* Info Produk - Gunakan text-foreground */}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-bold uppercase">
                    <span className={item.stok < 20 ? "text-red-600 dark:text-red-400" : "text-muted-foreground"}>
                      {item.status}
                    </span>
                    <span className="text-foreground">{item.stok}%</span>
                  </div>
                  {/* Progress bar akan mengikuti tema secara otomatis */}
